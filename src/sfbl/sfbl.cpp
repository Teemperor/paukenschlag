...
sfbl::WorldLight::WorldLight(b2World* world, sf::RenderWindow* window)
{
    ...
    this->_world = world;
    this->_window = window;
    lightShader.loadFromMemory(lightFragmentShader, sf::Shader::Fragment);
...
void sfbl::WorldLight::addAsPointLight(Light* light, sf::Uint16 rays){
    ...
    {
        float x = (float) (cos(perRay * i) * light->radius + pos.x);
        float y = (float) (sin(perRay * i) * light->radius + pos.y);
...
void sfbl::WorldLight::addAsConeLight(Light*light, sf::Uint16 rays, float cutOffAngle, float rotation){
    ...
    for (size_t i = 0; i <= rays; i++)
    {
        float x = (float) (cos((perRay * i) - cutOffAngle + rotation) * light->radius + pos.x);
        float y = (float) (sin((perRay * i) - cutOffAngle + rotation) * light->radius + pos.y);
        array.append(sf::Vertex(sf::Vector2f(x , y ), endColor));
    }
    lightMesh.push_back(array);
    fixedMesh.push_back(array);
...
void sfbl::WorldLight::addAsConeLight(sf::Uint16 rays, float cutOffAngle, float rotation, float radius, sf::Vector2f pos, sf::Color color){
    ...
    for (size_t i = 0; i <= rays; i++)
    {
        float x = (float) (cos((perRay * i) - cutOffAngle + rotation) * radius + pos.x);
        float y = (float) (sin((perRay * i) - cutOffAngle + rotation) * radius + pos.y);
        array.append(sf::Vertex(sf::Vector2f(x , y ), endColor));
    }
    lightMesh.push_back(array);
    fixedMesh.push_back(array);
    ...
    for (size_t i = 0; i < _lights.size(); i++)
    {
        if (_lights[i] == &light[0]){
            ...
            for (size_t o = 1; o < fixedMesh[i].getVertexCount(); o++)
            {
                ...
                fixedMesh[i][o].position.x = (float) (cos(perRay * (o - 1)) * light->radius + pos.x);
                fixedMesh[i][o].position.y = (float) (sin(perRay * (o - 1)) * light->radius + pos.y);
            }
            break;
        }
    }
...
void sfbl::WorldLight::updateConeLight(Light* light, float cutOffAngle, float rotation){
    ...
    for (size_t i = 0; i < _lights.size(); i++)
    {
        if (_lights[i] == &light[0]){
            ...
            {
                fixedMesh[i][o].color = endColor;
                fixedMesh[i][o].position.x = (float) (cos(perRay * (o - 1) - cutOffAngle + rotation) * light->radius + pos.x);
                fixedMesh[i][o].position.y = (float) (sin(perRay * (o - 1) - cutOffAngle + rotation) * light->radius + pos.y);
            }
            break;
        }
    }
    ...
    for (size_t m = 0; m < fixedMesh.size(); m++)
    {
        ...
        for (size_t i = 1; i < count; i++)
        {
            ...
            if (callBack.m_hit){
                ...
                lightMesh[m][i].color.a = (sf::Uint8) (255 * callBack.m_fraction);
            }
        }
    }
...
void sfbl::WorldLight::render(){
    ...
    {
        if(isPointLight[i]){
            sf::Vector2f screenPos = _lights[i]->position;
            screenPos.x *= SCALE;
            screenPos.y *= SCALE;
            lightShader.setParameter("lightPos", screenPos);
            lightShader.setParameter("lightColor",_lights[i]->color);
            lightShader.setParameter("radius", _lights[i]->radius * SCALE);
            sf::VertexArray array = lightMesh[i];

            for (std::size_t j = 0; j < array.getVertexCount(); j++) {
                array[j].position.x *= SCALE;
                array[j].position.y *= SCALE;
            }

            _render.draw(array, states);
        }
        else{
            sf::VertexArray array = lightMesh[i];
            for (std::size_t j = 0; j < array.getVertexCount(); j++) {
            ...
            _render.draw(array, sf::BlendAdd); // we can get spotlight effect just by disabling shader
        }
    }
    _render.display();
...
float32 RaysCastCallback::ReportFixture(b2Fixture* fixture, const b2Vec2& point, const b2Vec2&, float32 fraction) {
    float dist = b2Distance(startPoint, point);

    if (fixture->GetUserData()) {
        GameObject* object = static_cast<GameObject*>(fixture->GetUserData());
        if (!object->blocksView())
            return 1;
    }
